...
def run():
    ...
    if checkpoint == 0:
        ...
        ## mpileup depth to samtools depth at the junction is way lower than the ratio for the overall gene
            if "Sb6_wzx" in Hits:
                if "wbaM" in Hits:
                    a = get_ipython().getoutput('samtools depth -r wbaM:252-253 $outputbam | cut -f3')
                    a = [int(i) for i in a]; average_a = sum(a)/len(a)
                    if a == 0:
                        prediction = "Shigella boydii serotype 6 or 10"
                    else:
                        b = get_ipython().getoutput('cat $outputmpileup | awk \'$1=="wbaM" && $2 >251 && $2 <254\' | cut -f4')
                        b = [int(i) for i in b]; average_b = sum(b)/len(b)
                        junction_ratio = average_b/average_a
                        c = get_ipython().getoutput('samtools depth -r wbaM $outputbam | cut -f3')
                        c = [int(i) for i in c]; average_c = sum(c)/len(c)
                        d = get_ipython().getoutput('cat $outputmpileup | awk \'$1=="wbaM"\' | cut -f4')
